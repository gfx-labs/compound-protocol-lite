...
import {
  ...
  getCTokenDelegateAddress,
} from "../ContractLookup";

export async function getCTokenDelegateV(
  world: World,
  event: Event
...
async function cTokenDelegateAddress(
  ...
  return new AddressV(cTokenDelegate.address);
}
...
export async function getCTokenDelegateValue(
  world: World,
  event: Event
): Promise<Value> {
